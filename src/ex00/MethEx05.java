...
// 오버로딩 = over-loading = 과적재 : 함수이름이 동일해도 매개변수 타입이나 순서가 다르거나 갯수가 다르면 다른 함수로 인식하게 해주는 기법(문법)

/**
 * 1. 메서드는 동일한 이름으로 만들 수 없다.
 * 2. 단, 매개변수의 타입이 다르거나 위치가 다르거나 갯수가 다르면 다른 메서드로 인식 = 오버로딩
 * 3. 개발자 입장에서 함수이름을 하나만 기억하면 된다 (장점)
 * 4. 경우의 수가 너무 많아지면 퇴사하고 싶어진다.(단점) = 너무 많은 메서드가 필요하다.
 * 5. 어떨 때 사용하면 되냐? 갯수가 적을 때!! 20개안!! 10개 안!!
 * 6.
 */
